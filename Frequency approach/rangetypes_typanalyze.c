					...
					int samplerows, double totalrows)
{
	...
	lowers_cnt = (RangeBound *) palloc(sizeof(RangeBound) * samplerows);
	uppers_cnt = (RangeBound *) palloc(sizeof(RangeBound) * samplerows);
	...
	for (range_no = 0; range_no < samplerows; range_no++)
	{
		...
		if (!empty)
		{
			...
			lowers_cnt[non_empty_cnt] = lower;
			uppers_cnt[non_empty_cnt] = upper;
	...
	if (non_null_cnt > 0)
	{
		...
		if (non_empty_cnt >= 2)
		{
			...
			for (i = 0; i < num_hist; i++)
			{
				...
				{
					lower_bound = lowers_cnt[r].val;
					upper_bound = uppers_cnt[r].val;
					// if overlaps
